    ...
    def __str__(self) -> str:
        ...
        for i in self.items:
            if i.status == ReportItemStatus.PASSED:
                ...
                ret += f"{i.key}: {i.status.name} (source: {i.src_val} -- target: {i.target_val})\n"
...
class VerificationSet(object):
    def __init__(self) -> None:
        self.items: Dict[str, int | str | List] = dict()
...
def get_verification_set_by_uri(mongo_uri: str, db_name: str) -> VerificationSet:
    ...
    set = VerificationSet()
    set.add_item("collections_names", coll_names)

    for name in coll_names:
        pfx = f"collection_{name}"
        stats = db.command("collStats", name)
        count = stats['count']
        set.add_item(f"{pfx}.doc_count", count)
        set.add_item(f"{pfx}.indexes",
                     get_indexes(stats['indexSizes']))
        set.add_item(f"{pfx}.first_obj",
                     find_one(db[name], pymongo.ASCENDING))
        set.add_item(f"{pfx}.last_obj",
